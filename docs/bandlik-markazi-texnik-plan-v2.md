...
> **v2 — ko'rib chiqishdan keyingi tuzatishlar:** (1) hujjat fayllari maxfiyligi — `/media/` emas, autentifikatsiyalangan view + AI'ga base64/ichki URL; (2) real AI'da inline emas, `process_documents` cron; (3) FSM persist — custom aiogram `BaseStorage`; (4) follow-up yetkazish — backend → Telegram Bot API; (5) `no_interview` outcome + dedup uchun `idempotency_key`; (+) public `/l/` view throttling.
...
class FollowUp(BaseModel):
    ...
    class Outcome(models.TextChoices):
        ...
        NO_CONTACT = "no_contact"; NO_INTERVIEW = "no_interview"   # Q2 (suhbat) 3x "Yo'q" terminali
...
- `access.py` — **public** `/l/{token}` view (auth'siz):
  ...
  - **Throttling:** public view (`AllowAny`) DRF throttle bilan himoyalanadi (token enumeration / spam'ga qarshi); token UUIDv4 (taxminlab bo'lmaydi).
...
- `views.py`:
  ...
  - `POST /api/v1/bot/document` (multipart, **service token**) → `Document(status=pending)` + `ai_gateway.analyze()` (`green`→`verified`, `yellow/red`→`flagged`).
    - **Stub uchun inline mayli** (stub bir zumda javob beradi). **Real modelga o'tilganda** → upload darhol `pending` qaytarsin, AI'ni `process_documents` management command (Faza E cron bilan bir xil naqsh) ishlatsin — bot upload so'rovi 30s bloklab qolmasin.
  - **Maxfiylik:** `documents/` fayllari (CV/IELTS — PII) ochiq `/media/` orqali **berilmaydi**. Hujjat autentifikatsiyalangan view (yoki imzolangan/vaqtinchalik URL) orqali beriladi; AI servisiga ochiq URL emas — **ichki URL yoki base64** uzatiladi.
- `server/ai_gateway/client.py`:
  ...
  def analyze(document):
      ...
      payload = {"document_id": str(document.id), "doc_type": document.type,
                 "file_b64": _read_b64(document.file),   # ochiq URL emas — base64 yoki ichki URL
...
- **Tavsiya (asosiy):** botdan `idempotency_key` (submission UUID) qabul qilib, takror kalitni rad etish — constraint olib tashlangach DB darajasida himoya qolmagani uchun ishonchli yo'l shu. Zaxira: "oxirgi 60s + bir xil employment_status" tekshiruvi (snapshot model uchun nodir dub ham qabul qilinadi).
...
- E.2 — `server/crm/management/commands/process_followups.py`:
  - `FollowUp.objects.filter(next_send_at__lte=now, stage__in=[pending,contacted,interviewed], flagged_for_staff=False)` → talabaga savol yuborish.
  - **Yetkazish mexanizmi:** backend cron Telegram'ga to'g'ridan-to'g'ri **Telegram Bot API** orqali yuboradi (token backend'da) — eng soddasi, botga bog'liqlik yo'q. Muqobil: bot ichki HTTP endpoint ochadi va backend chaqiradi. (Bittasi tanlansin; `process_followups` shunga tayanadi.)
  - Javobga ko'ra kadens: 2→5→7 kun; 3× "Yo'q" → `stage=done, flagged_for_staff=true` va `outcome` = (`contacted` → `no_contact` | `interviewed` → `no_interview`).
...
- **FSM persist:** hozir `MemoryStorage` → restart'da holat yo'qoladi. Tavsiya: aiogram'ning **custom `BaseStorage`** ini yozib, u `Bot2Student.state` (B.3) orqali backend'da saqlasin — "har qadamda qo'lda yangilash"dan ko'ra ishonchliroq. Qayta `/start` davom ettiradi. (D fazasida verify bilan birga qilinadi.)
